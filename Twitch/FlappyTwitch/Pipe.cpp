...
void Pipe::Initialize(bool obstacle)
{
	if (pipeSprite == NULL) {
		pipeSprite = new Sprite("Assets/Art/road.png");
		pipeSprite->SetScale(Vector3(0.1f, 0.35f, 1));
	}
	else if (obstacle) {
		pipeSprite->SetScale(Vector3(0.1f, 0.05f, 1));
	}
	else {
		pipeSprite->SetScale(Vector3(0.1f, 0.35f, 1));
	}
...
Pipe::Pipe(Vector3 _pos, int _points, bool _obstacle) : Pipe()
{
	if (!pipeSprite)
	{
		return;
	...
	topRB.AddForce(Vector3(-speed - (_points * 2), 0, 0));
	botRB.AddForce(Vector3(-speed - (_points * 2), 0, 0));
}
